...
### Lỗi: "You will need to use a browser that you don't launch from ZAP..." khi dùng ZAP trong Docker
Lỗi này xảy ra khi bạn dùng tính năng **Manual Explore** (gọi trình duyệt từ ZAP) nhưng ZAP đang chạy trong Docker nên nó mặc định tính năng gọi trình duyệt bị vô hiệu hóa. 

**Cách khắc phục (Bật trình duyệt nhúng):**
1. Trên giao diện ZAP Webswing, click vào biểu tượng **Bánh răng (Options)** trên thanh công cụ (hoặc menu **Tools** -> **Options**).
2. Tìm đến mục **Display** ở menu bên trái.
3. Tích chọn ô **Enable app integration in containers** (Cho phép tích hợp ứng dụng trong container).
4. Nhấn **OK**. 
5. Thử lại tính năng **Manual Explore**, ZAP sẽ tự động mở được trình duyệt Firefox (headless) tích hợp sẵn trong Docker image `zaproxy/zap-stable`.
