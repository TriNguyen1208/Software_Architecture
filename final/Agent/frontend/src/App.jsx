        ...
        for (let line of parts) {
          if (line.startsWith('data: ')) {
            ...
            if (data === '[DONE]') {
              ...
              const toolName = data.slice(13).trim();
              setMessages(prev => {
                const newMessages = [...prev];
                const lastMessage = { ...newMessages[newMessages.length - 1] };
                lastMessage.tools = lastMessage.tools || [];
                lastMessage.tools.push({ name: toolName, status: 'running' });
                newMessages[newMessages.length - 1] = lastMessage;
                return newMessages;
              });
            } else if (data.startsWith('[TOOL_END]')) {
              const toolName = data.slice(11).trim();
              setMessages(prev => {
                const newMessages = [...prev];
                const lastMessage = { ...newMessages[newMessages.length - 1] };
                if (lastMessage.tools) {
                  const toolIndex = lastMessage.tools.findLastIndex(t => t.name === toolName && t.status === 'running');
                  if (toolIndex !== -1) {
                    lastMessage.tools[toolIndex].status = 'done';
                  }
                }
                newMessages[newMessages.length - 1] = lastMessage;
                return newMessages;
                  ...
                  >
                    {msg.role === 'ai' && msg.tools && msg.tools.length > 0 && (
                      <div className="flex flex-col gap-2 mb-3">
                        {msg.tools.map((t, idx) => (
                          <div key={idx} className="flex items-center gap-2 text-xs font-medium text-slate-600 bg-slate-100/70 self-start px-3 py-1.5 rounded-full border border-slate-200 shadow-sm animate-in fade-in slide-in-from-left-2 duration-300">
                            {t.status === 'running' ? (
                            ...
                            <span>
                              {t.name === 'search_travel_info' ? 'Đang tra cứu cơ sở dữ liệu Chroma' :
                               t.name === 'get_weather_info' ? 'Đang lấy thông tin thời tiết' :
                               `Đang gọi công cụ ${t.name}`}
                              {t.status === 'running' ? '...' : ' - Hoàn tất'}
                            </span>
                          </div>
                        ))}
                      </div>
